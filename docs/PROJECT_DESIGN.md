...
PDF의 단원별 산출물을 별도 데모로 흩어 놓지 않고 하나의 완성된 날씨 앱으로 통합한다. 최종 앱에서 Vue 기본 문법, Composition API, 컴포넌트 통신, Router, Pinia, Axios, 외부 UI 라이브러리, 실제 API, 품질 검사와 Vercel 정적 배포가 모두 코드와 배포본에서 확인되어야 한다.
...
검수일: 2026-08-29. `[x]`는 로컬 코드·브라우저·Vercel 배포본 검증까지 완료한 항목이다.

### 제출과 문서

- [x] GitHub Public 저장소를 시크릿 창에서 로그인 없이 볼 수 있다.
- [x] Vercel 배포 URL이 있으며 직접 URL 접근/새로고침도 정상이다.
...
- [x] OpenWeatherMap 실제 현재 날씨가 화면에 표시된다. (Vercel Production 환경 변수 적용 완료)
- [x] OpenWeatherMap 추가 API인 5일 예보가 상세 화면에 표시된다. (상세 화면에서 확인 완료)
...
- [x] Vercel Production 환경에 `VITE_OPENWEATHER_API_KEY`가 설정되어 있다.
- [x] 배포본에서 홈, 상세 직접 접근, 404, API 요청, 단축키를 재검증했다.
